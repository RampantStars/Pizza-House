...
export const SizeCard = (size: Size) => {
  const { deleteSize, setEdit, setIsEdit } = useSizeStore(({ deleteSize, setEdit, setIsEdit }) => ({
    deleteSize,
    setEdit,
    setIsEdit,
  }));
  ...
  return (
    <div className={styles.sizeCard}>
      <h3>{size.name}</h3>
      <p className={styles.price}>Цена: {size.price} руб</p>
      <div className={styles.btn__container}>
        <button onClick={() => onEdit(size.id)} className={`${styles.btn} ${styles.btn__edit} `}>
          <svg
